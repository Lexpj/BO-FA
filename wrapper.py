...
class RDEIWrapper:
    def __init__(self, func, dim, ub, lb, total_budget, DoE_size, random_seed):
        ...
        import pathlib
        my_dir = pathlib.Path(__file__).parent.resolve()
        sys.path.append(os.path.join(my_dir, 'mylib', 'lib_RDEI/HEBO/RDEI'))
        print(sys.path)
        # sys.path.insert(0, bayes_bo_lib)
        # print(sys.path)
        self.func = func
...
class RDPIWrapper:
    def __init__(self, func, dim, ub, lb, total_budget, DoE_size, random_seed):
        ...
        import pathlib
        my_dir = pathlib.Path(__file__).parent.resolve()
        sys.path.append(os.path.join(my_dir, 'mylib', 'lib_RDPI/HEBO/RDPI'))
        print(sys.path)
        # sys.path.insert(0, bayes_bo_lib)
        # print(sys.path)
        self.func = func
...
def wrapopt(optimizer_name, func, ml_dim, ml_total_budget, ml_DoE_size, random_seed):
    ...
    if optimizer_name == 'RDUCB':
        return RDUCBWrapper(func=func, dim=ml_dim, ub=ub, lb=lb, total_budget=ml_total_budget, DoE_size=ml_DoE_size,
                             random_seed=random_seed)
    if optimizer_name == "RDEI":
        return RDEIWrapper(func=func, dim=ml_dim, ub=ub, lb=lb, total_budget=ml_total_budget, DoE_size=ml_DoE_size,
                             random_seed=random_seed)   
    if optimizer_name == "RDPI":
        return RDPIWrapper(func=func, dim=ml_dim, ub=ub, lb=lb, total_budget=ml_total_budget, DoE_size=ml_DoE_size,
                             random_seed=random_seed)   
